...
- 🔖 **1.0.0 release.** First stable release of this fork. No functional changes from 0.9.21; the version is bumped to mark the fork's own release line.
- 🚫 **PyPI publishing disabled.** The `Publish to PyPI` workflow no longer runs automatically and its publish job is gated on the upstream repository owner, so this fork never attempts to publish the shared `cptr` package.
